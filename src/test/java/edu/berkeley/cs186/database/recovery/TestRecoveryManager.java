...
public class TestRecoveryManager {
    ...
    public void testStudentIntegration() throws Exception {
        ...
        setupRedoChecks(Arrays.asList(
                (LogRecord record) -> assertEquals((long) LSNs.get(1), (long) record.LSN),
        ...
        setupRedoChecks(Arrays.asList(
                (LogRecord record) -> {
                    assertEquals(LogType.UNDO_FREE_PAGE, record.getType());
                ...
                (LogRecord record) -> {
                    ...
                    assertNotNull("log record not appended to log yet", record.LSN);
                    assertEquals((long) record.LSN, transactionTable.get(3L).lastLSN);
    ...
    public void testStudentAnalysis() throws Exception {
        ...
        LSNs.add(logManager.appendToLog(new CommitTransactionLogRecord(1L, LSNs.get(1)))); // 3
        LSNs.add(logManager.appendToLog(new EndTransactionLogRecord(1L, LSNs.get(3)))); // 4
        LSNs.add(logManager.appendToLog(new CommitTransactionLogRecord(3L, LSNs.get(2)))); // 5
        LSNs.add(logManager.appendToLog(new EndTransactionLogRecord(3L, LSNs.get(5)))); // 6
        LSNs.add(logManager.appendToLog(new FreePageLogRecord(2L, 10000000001L, 0L))); // 7
        LSNs.add(logManager.appendToLog(new AbortTransactionLogRecord(2L, LSNs.get(7)))); // 8
        LSNs.add(logManager.appendToLog(new BeginCheckpointLogRecord(9876543210L))); // 9
        ...
        assertEquals(Transaction.Status.RECOVERY_ABORTING, transaction2.getStatus());
        assertFalse(transaction2.cleanedUp);
        assertEquals(Transaction.Status.COMPLETE, transaction3.getStatus());
        assertTrue(transaction3.cleanedUp);
    }
    ...
    public void testStudentRedo() throws Exception {
        ...
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(1L, 10000000003L, LSNs.get(2), (short) 2,
                before, after))); // 3
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(1L, 10000000003L, LSNs.get(3), (short) 2,
                after, before))); // 4
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(1L, 10000000003L, LSNs.get(4), (short) 2,
                before, after))); // 5
        LSNs.add(logManager.appendToLog(new AllocPartLogRecord(1L, 10, LSNs.get(5)))); // 6
        LSNs.add(logManager.appendToLog(new CommitTransactionLogRecord(1L, LSNs.get(6)))); // 7
        LSNs.add(logManager.appendToLog(new EndTransactionLogRecord(1L, LSNs.get(7)))); // 8
        ...
        setupRedoChecks(Arrays.asList(
                ...
                (LogRecord record) -> assertEquals((long) LSNs.get(6), (long) record.LSN)
        ));
    ...
    public void testStudentUndo() throws Exception {
        ...
        List<Long> LSNs = new ArrayList<>();
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(1L, 10000000008L, 0L, (short) 4,
                before, after))); // 0
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(2L, 10000000007L, 0L, (short) 3,
                before, after))); // 1
        LSNs.add(logManager.appendToLog(new UpdatePageLogRecord(2L, 10000000006L, LSNs.get(1), (short) 2,
                before, after))); // 2
        LSNs.add(logManager.appendToLog(new AbortTransactionLogRecord(1L, LSNs.get(0)))); // 3
        LSNs.add(logManager.appendToLog(new AbortTransactionLogRecord(2L, LSNs.get(2)))); // 4
        ...
        entry1.touchedPages = new HashSet<>(Arrays.asList(10000000008L));
        entry1.transaction.setStatus(Transaction.Status.RECOVERY_ABORTING);
        entry2.lastLSN = LSNs.get(4);
        entry2.touchedPages = new HashSet<>(Arrays.asList(10000000007L, 10000000006L));
        ...
        setupRedoChecks(Arrays.asList(
                (LogRecord record) -> {
                    ...
                    assertNotNull("log record not appended to log yet", record.LSN);
                    assertEquals((long) record.LSN, transactionTable.get(2L).lastLSN);
                    assertEquals(Optional.of(10000000006L), record.getPageNum());
                },
                (LogRecord record) -> {
                    assertEquals(LogType.UNDO_UPDATE_PAGE, record.getType());
                    assertNotNull("log record not appended to log yet", record.LSN);
                    assertEquals((long) record.LSN, transactionTable.get(2L).lastLSN);
                    assertEquals(Optional.of(10000000007L), record.getPageNum());
                },
                (LogRecord record) -> {
                    assertEquals(LogType.UNDO_UPDATE_PAGE, record.getType());
                    assertNotNull("log record not appended to log yet", record.LSN);
                    assertEquals((long) record.LSN, transactionTable.get(1L).lastLSN);
                    assertEquals(Optional.of(10000000008L), record.getPageNum());
                }
        ));
